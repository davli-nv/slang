...
void dumpIRIfEnabled(
    ...
    char const* label)
{
    if (true||codeGenContext->shouldDumpIR())
    {
        ...
        String dumpFileName = (String("d:\\slang\\dump\\ir\\dump-") + label + "." + targetFormatStr + ".txt");
...
SlangResult CodeGenContext::emitEntryPointsSourceFromIR(ComPtr<IArtifact>& outArtifact)
{
    ...
    String dumpFileName = (String("d:\\slang\\dump\\ir\\dump-EMIT ENTRY POINTS SOURCE.") + targetFormatStr + lastDumpedEntryPoint + ".src.txt");
    FILE* f = nullptr;
    fopen_s(&f, dumpFileName.getBuffer(), "wt");
    ...
    ComPtr<IArtifact>& dbgArtifact)
{
...

    {
        ...
        char targetFormatStr[32];
        sprintf(targetFormatStr, "%d", codeGenContext->getTargetFormat());
        String dumpFileName = (String("d:\\slang\\dump\\ir\\dump-UNOPT SPIRV.") + targetFormatStr + lastDumpedEntryPoint + ".spv");
        FILE* f = nullptr;
    ...
    IDownstreamCompiler* compiler = codeGenContext->getSession()->getOrLoadDownstreamCompiler(
        ...
        codeGenContext->getSink());
    static volatile bool opt = false;
    ...
    if (opt&&compiler)
    {
...

        {
            ...
            char targetFormatStr[32];
            sprintf(targetFormatStr, "%d", codeGenContext->getTargetFormat());
            String dumpFileName = (String("d:\\slang\\dump\\ir\\dump-OPT SPIRV.") + targetFormatStr + lastDumpedEntryPoint + ".spv");
            FILE* f = nullptr;
            fopen_s(&f, dumpFileName.getBuffer(), "wb");
