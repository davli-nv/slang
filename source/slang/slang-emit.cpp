    ...
    char const* label)
{
    if (true||codeGenContext->shouldDumpIR())
    {
        ...
        String dumpFileName = (String("c:\\slang\\ir\\dump-") + label + "." + targetFormatStr + ".txt");
...
SlangResult CodeGenContext::emitEntryPointsSourceFromIR(ComPtr<IArtifact>& outArtifact)
{
    ...
    String dumpFileName = (String("c:\\slang\\ir\\dump-EMIT ENTRY POINTS SOURCE.") + targetFormatStr + lastDumpedEntryPoint + ".src.txt");
    FILE* f = nullptr;
    fopen_s(&f, dumpFileName.getBuffer(), "wt");
    ...
    ComPtr<IArtifact>& dbgArtifact)
{
...

    {
        ...
        char targetFormatStr[32];
        sprintf(targetFormatStr, "%d", codeGenContext->getTargetFormat());
        String dumpFileName = (String("c:\\slang\\ir\\dump-UNOPT SPIRV.") + targetFormatStr + lastDumpedEntryPoint + ".spv");
        FILE* f = nullptr;
    ...
    if (opt&&compiler)
    {
...

        {
            ...
            char targetFormatStr[32];
            sprintf(targetFormatStr, "%d", codeGenContext->getTargetFormat());
            String dumpFileName = (String("c:\\slang\\ir\\dump-OPT SPIRV.") + targetFormatStr + lastDumpedEntryPoint + ".spv");
            FILE* f = nullptr;
            fopen_s(&f, dumpFileName.getBuffer(), "wb");
